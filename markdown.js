...
function parseBlocks(lines, start, end) {
  ...
  while (i < end) {
    ...
    if (fenceM) {
      ...
      const highlighted = lang ? syntaxHighlight(escapeHtml(code.replace(/\n$/, '')), lang)
    ...
    if (hM) {
      ...
      html += `<h${level} id="${escapeHtml(id)}">${parseInline(text)}</h${level}>\n`;
      i++;
      continue;
    }
    ...
    if (line.startsWith('>')) {
      const quoteLines = [];
      while (i < end && (lines[i].startsWith('>') || (lines[i].trim() !== '' && quoteLines.length > 0))) {
    ...
    // Table
    if (line.includes('|') && i + 1 < end && /^\s*\|?[\s:-]+\|/.test(lines[i + 1])) {
    ...
    while (i < end) {
      ...
      if (/^(?:\*\s*){3,}$|^(?:-\s*){3,}$|^(?:_\s*){3,}$/.test(ln.trim())) break;
      if (ln.startsWith('>')) break;
      if (/^\s*[-*+]\s/.test(ln) || /^\s*\d+\.\s/.test(ln)) break;
      if (ln.includes('|') && i + 1 < end && /^\s*\|?[\s:-]+\|/.test(lines[i + 1])) break;
      paraLines.push(ln);
      i++;
    }
  ...
  while (i < endI) {
    ...
    if (indent === baseIndent && (ulM || olM)) {
      ...
      while (i < endI) {
        ...
        if (nl.trim() === '') {
          ...
          if (j < endI) {
            ...
            if (ni > baseIndent) { i++; continue; }
          }
          break;
        }
        const ni = (nl.match(/^(\s*)/) || ['', ''])[1].length;
        if (ni <= baseIndent && (/^\s*[-*+]\s/.test(nl) || /^\s*\d+\.\s/.test(nl))) break;
        if (ni > baseIndent) { itemLines.push(nl); i++; }
        else break;
      }
      ...
      for (let k = 1; k < itemLines.length; k++) {
        if (/^\s*[-*+]\s/.test(itemLines[k]) || /^\s*\d+\.\s/.test(itemLines[k])) {
          nestedStart = k; break;
        }
      }
      ...
      let itemHtml;
      if (nestedStart > 0) {
        const textPart = parseInline(itemLines.slice(0, nestedStart).join(' ').trim());
...
function parseTable(lines, startI, endI) {
  ...
  const headers = splitTableRow(lines[i]); i++;
...
function syntaxHighlight(code, lang) {
  ...
  const langMap = {
    ...
    python: 'py', golang: 'go', shell: 'bash', sh: 'bash',
  };
  ...
  code = code.replace(/(&quot;)((?:\\.|(?!&quot;).)*?)(&quot;)|(['`])((?:\\.|(?!\4).)*?)\4/g,
    (m) => protect(`<span class="tok-str">${m}</span>`));

  // Block comments
  code = code.replace(/(\/\*[\s\S]*?\*\/)/g, (m) => protect(`<span class="tok-cmt">${m}</span>`));

  // Line comments — `//` everywhere; `#` only for py/bash
  code = code.replace(/\/\/[^\n]*/g, (m) => protect(`<span class="tok-cmt">${m}</span>`));
  if (key === 'py' || key === 'bash') {
    code = code.replace(/#[^\n]*/g, (m) => protect(`<span class="tok-cmt">${m}</span>`));
  }
  ...
  code = code.replace(/\b\d+(?:\.\d+)?(?:[eE][+-]?\d+)?\b/g, (m) => protect(`<span class="tok-num">${m}</span>`));
  code = code.replace(keywords[key], '<span class="tok-kw">$1</span>');

  // Restore protected regions
  return code.replace(/[\uE000-\uF8FF]/g, (ch) => stash[ch.charCodeAt(0) - 0xE000]);
}
